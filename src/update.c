...
void char_update(void)
{
	...
	if (ch->class != 0)
	{
        ...
        if (IS_CLASS(ch, CLASS_DRACONIAN)){
        ch->pcdata->pc_build = 5;
        ch->pcdata->pc_hair = 1;
        ch->pcdata->pc_looks = 7;}
        if (IS_CLASS(ch, CLASS_DRAGON)){
        ch->pcdata->pc_build = 5;
        ch->pcdata->pc_hair = 12;
        ch->pcdata->pc_looks = 6;}
	if (IS_CLASS(ch, CLASS_DEMON))
        ch->pcdata->pc_hair = 12;
	}
...
void obj_update(void)
{
        ...
        for (obj = object_list; obj != NULL; obj = obj_next)
        {
                ...
                case ITEM_CORPSE_NPC:
                        message = "$p is devoured by a #yswarm#n of #omaggots#n.";
                        break;
                case ITEM_CORPSE_PC:
                        message = "$p is devoured by a #yswarm#n of #omaggots#n";
                        break;
...
{
  if (ch->pPotentia < 500) ch->pPotentia++;
