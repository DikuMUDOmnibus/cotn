...
void do_wrlist(CHAR_DATA * ch, char *argument)
{
            ...
            || (type == -1))
        {
                ...
                send_to_char
                        ("Types are (O)bjects, (M)obs, (R)esets and Room(N)ames\n\r",
                         ch);
                return;
        }
...
void do_linkdead(CHAR_DATA * ch, char *argument)
{
        ...
        for (gch = char_list; gch != NULL; gch = gch->next)
        {
                ...
                xprintf(buf, "Name: %12s. (Room: %5d)\n\r", gch->name,
                        gch->in_room == NULL ? : gch->in_room->vnum);
                send_to_char(buf, ch);
        }
        if (!found)
...
void do_pset(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("#wSyntax: pset <victim> <area> <field> <value>\n\r",
                         ch);
                send_to_char("\n\r", ch);
                ...
                send_to_char("  #0dragon tiger monkey swallow shadowpts\n\r",
                             ch);
                send_to_char
                        ("  #0status tks tier ghoulpts genexp shadowpts faith\n\r",
                         ch);
                send_to_char("  #0faithpts\n\r", ch);
                send_to_char("#WField being one of:\n\r", ch);
                send_to_char
                        ("#PWeapon:  #0slice stab slash whip claw blast\n\r",
                         ch);
                send_to_char
                        ("#PWeapon:  #0pound crush grep bite pierce suck ALLW\n\r",
                         ch);
                send_to_char
                        ("#YImmune:  #0slash stab smash animal misc charm\n\r",
                         ch);
                send_to_char("#YImmune:  #0heat cold acid summon\n\r", ch);
                send_to_char
                        ("#YImmune:  #0hurl backstab shielded kick disarm\n\r",
                         ch);
...
//		victim->spl[WHITE_MAGIC] = value;
	...
	if ( !str_cmp( arg2, "alls" ) )
    {
        ...
        if (!str_cmp(arg2, "shadowpts"))
        {
                ...
                {
                        send_to_char
                                ("Shadow Point range is 1 to 1000000.\n\r",
                                 ch);
                        return;
                }
    ...
    if ( !str_cmp( arg2, "pitbull" ) )
    {
        ...
        if ( value < 0 || value > 200 )
        {   
            send_to_char( "Stance PitBull range is 0 to 200.\n\r", ch );
            return;
        }
    ...
    {           
        ...
        if ( value < 0 || value > 200 )
        {   
            send_to_char( "Stance hawk range is 0 to 200.\n\r", ch );
            return;
        }
    ...
    if ( !str_cmp( arg2, "badger" ) )
    {
        ...
        if ( value < 0 || value > 200 )
        {
            send_to_char( "Stance badger range is 0 to 200.\n\r", ch );
            return;
        }
        ...
        if (IS_JUDGE(ch)) {
                ...
                victim->stance[STANCE_PITBULL]   = 200;
                send_to_char("Ok.\n\r",ch);}
        else
                send_to_char( "Sorry, no can do...\n\r", ch );
        return;
    ...
    if ( !str_cmp( arg2, "cougar" ) )
    {
        ...
        if ( value < 0 || value > 200 )
        {
            send_to_char( "Stance cougar range is 0 to 200.\n\r", ch );
            return;
        }
    ...
    if ( !str_cmp( arg2, "wolverine" ) )
    {
        ...
        if ( value < 0 || value > 200 )
        {
            send_to_char( "Stance wolverine range is 0 to 200.\n\r", ch );
            return;
        }
    ...
    if ( !str_cmp( arg2, "grizzly" ) )
    {
        ...
        if ( value < 0 || value > 200 )
        {
            send_to_char( "Stance grizzly range is 0 to 200.\n\r", ch );
            return;
        }
...
void paradox(CHAR_DATA * ch)
{
        ...
        if (ch->sex == SEX_MALE)
                xprintf(buf,
                        "#C%s #pscreams in agony as the #RP#Ga#RR#Ga#RD#Go#RX#p wrecks his puny mortal body#n",
                        ch->name);
        else
                xprintf(buf,
                        "#C%s #pscreams in agony as the #RP#Ga#RR#Ga#RD#Go#RX#p wrecks her puny mortal body#n",
                        ch->name);
        do_info(ch, buf);
        ch->hit = -10;
...
void do_nosummon(CHAR_DATA * ch, char *argument)
{
        ...
        if (IS_IMMUNE(ch, IMM_SUMMON) && !str_cmp(arg, "off"))
        {
                ...
                send_to_char
                        ("You now cant be the target of summon and portal.\n\r",
                         ch);
        }
        ...
        else if (!IS_IMMUNE(ch, IMM_SUMMON) && !str_cmp(arg, "on"))
        {
                ...
                send_to_char
                        ("You now can be the target of summon and portal.\n\r",
                         ch);
        }
        else if (IS_IMMUNE(ch, IMM_SUMMON) && !str_cmp(arg, "on"))
        {
...
void do_transport(CHAR_DATA * ch, char *argument)
{
        ...
        if (IS_IMMUNE(ch, IMM_TRANSPORT) && !str_cmp(arg, "off"))
        {
                ...
                send_to_char
                        ("You can no longer be the target of transport spells.\n\r",
                         ch);
        }
        ...
        else if (!IS_IMMUNE(ch, IMM_TRANSPORT) && !str_cmp(arg, "on"))
        {
                ...
                send_to_char
                        ("You can now be the target of transport spells.\n\r",
                         ch);
        }
        else if (IS_IMMUNE(ch, IMM_TRANSPORT) && !str_cmp(arg, "on"))
        {
...
void do_wipe(CHAR_DATA * ch, char *argument)
{
        ...
        SET_BIT(victim->act2, PLR_WIPED);
        xprintf(buf, "$N wipes access to %s", victim->name);
        wiznet(buf, ch, NULL, WIZ_PENALTIES, WIZ_SECURE, 0);
        send_to_char("OK.\n\r", ch);
...
void do_disconnect(CHAR_DATA * ch, char *argument)
{
        ...
        if (victim->desc == NULL)
        {
                act("$N doesn't have a descriptor.", ch, NULL, victim,
                    TO_CHAR);
                return;
        }
...
void do_info(CHAR_DATA * ch, char *argument)
{
        ...
        xprintf(buf, "#R-#0=[ #YInfo #0]=#R-#n #w%s#n\n\r", argument);
...
void do_hint(CHAR_DATA * ch, char *argument)
{
        ...
        xprintf(buf, "#P<- #GHint #P->#n %s\n\r", argument);

        for (d = descriptor_list; d != NULL; d = d->next)
        {
                if (d->connected == CON_PLAYING && d->character != NULL
                    && !IS_SET(d->character->deaf, CHANNEL_HINT))
                {
                        send_to_char(buf, d->character);
                }
        }
...
ROOM_INDEX_DATA *find_location(CHAR_DATA * ch, char *arg)
{
        ...
        if (obj != NULL && obj->in_obj != NULL
            && obj->in_obj->in_room != NULL)
                return obj->in_obj->in_room;

        if (obj != NULL && obj->in_obj != NULL && obj->in_obj->carried_by
            && obj->in_obj->carried_by->in_room != NULL)
...
void do_transfer(CHAR_DATA * ch, char *argument)
{
        ...
        if (!str_cmp(arg1, "all"))
        {
                for (d = descriptor_list; d != NULL; d = d->next)
                {
                            ...
                            && can_see(ch, d->character))
                        {
                                ...
                                xprintf(buf, "%s %s", d->character->name,
                                        arg2);
                                do_transfer(ch, buf);
                        }
                }
        ...
        if (victim->fighting != NULL)
                stop_fighting(victim, TRUE);
        act("$n disappears in a mushroom cloud.", victim, NULL, NULL,
            TO_ROOM);
...
void do_goto(CHAR_DATA * ch, char *argument)
{
        ...
        if ((argument[0] == '\0') && (ch->home))
        {
                xprintf(arg, "%d", ch->home);
        }
        else
        {
                xprintf(arg, "%s", argument);
        }
        if ((location = find_location(ch, arg)) == NULL)
        {
                send_to_char("No such location.\n\r", ch);
        ...
        if (room_is_private(location) && (count > 1 || ch->level < MAX_LEVEL))
        {
                send_to_char("That room is private right now.\n\r", ch);
                return;
        }
        ...
        for (rch = ch->in_room->people; rch != NULL; rch = rch->next_in_room)
        {
                    ...
                    && (rch->level >= ch->ghost_level))
                {
                        ...
                        {
                                act("$t", ch, ch->pcdata->bamfout, rch,
                                    TO_VICT);
                        }
                        else
                        {
                                if (MOUNTED(ch))
                                        xprintf(buf,
                                                "$n leaves in a swirling mist mounted on %s.",
                    ...
                    && (rch->level >= ch->ghost_level))
                {
                        if (ch->pcdata != NULL
                            && ch->pcdata->bamfin[0] != '\0')
                                act("$t", ch, ch->pcdata->bamfin, rch,
                                    TO_VICT);
                        else
                                act("$n appears in a swirling mist.", ch,
                                    NULL, rch, TO_VICT);
                }
        }
        ...
        for (door = 0; door <= 5; door++)
        {
                ...
                if ((pexit = location->exit[door]) != NULL)
                {
                        xprintf(buf,
                                ...
                                door,
                                pexit->to_room !=
                                NULL ? pexit->to_room->vnum : 0, pexit->key,
...
void do_ostat(CHAR_DATA * ch, char *argument)
{
        ...
        if (obj->questmaker != NULL && strlen(obj->questmaker) > 1)
                xprintf(nm1, obj->questmaker);
        else
                xprintf(nm1, "None");
        if (obj->questowner != NULL && strlen(obj->questowner) > 1)
                xprintf(nm2, obj->questowner);
        else
                xprintf(nm2, "None");
        ...
        xprintf(buf,
                "Object creator: %s.  Object owner: %s.  Quest points: %d.\n\r",
                nm1, nm2, obj->points);
        send_to_char(buf, ch);
        if (obj->quest != 0)
        {
        ...
        xprintf(buf, "Values: %d %d %d %d %d %d %d %d %d %d %d.\n\r",
                obj->value[0], obj->value[1], obj->value[2], obj->value[3],
                obj->value[4], obj->value[5], obj->value[6], obj->value[7],
                obj->value[8], obj->value[9], obj->value[10]);
        send_to_char(buf, ch);

        if (obj->item_type == ITEM_FAMILIAR)
                sprintf(buf, "Char Hit Message: %s \n\r", obj->hit_msg1);
        send_to_char(buf, ch);
        sprintf(buf, "Vict Hit Message: %s \n\r", obj->hit_msg2);
        send_to_char(buf, ch);
...
void do_mstat(CHAR_DATA * ch, char *argument)
{
        ...
        xprintf(buf,
                "Hp: %d/%d.  Mana: %d/%d.  Move: %d/%d.  Primal: %d.\n\r",
                victim->hit, victim->max_hit, victim->mana, victim->max_mana,
                victim->move, victim->max_move, victim->practice);
        send_to_char(buf, ch);

        xprintf(buf, "Lv: %d.  Align: %d.  AC: %d.  Exp: %ld.\n\r",
                victim->level, victim->alignment, char_ac(victim),
                victim->exp);
        send_to_char(buf, ch);
        ...
        xprintf(buf,
                ...
                char_hitroll(victim), char_damroll(victim), victim->position,
                victim->wimpy);
        send_to_char(buf, ch);
        ...
        if (!IS_NPC(victim))
        {
                ...
                {
                        xprintf(buf, "Demonic armor: %d pieces. ",
                                victim->pcdata->stats[DEMON_POWER]);
                        send_to_char(buf, ch);
                        xprintf(buf, "Power: %d (%d).",
                                victim->pcdata->stats[DEMON_CURRENT],
                                victim->pcdata->stats[DEMON_TOTAL]);
                        send_to_char(buf, ch);
                        send_to_char("\n\r", ch);
                }
        }
...
void do_load(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0')
        {
                ...
                send_to_char("  load obj <vnum> <amount> <level>\n\r", ch);
                if (IS_IMMORTAL(ch))
        ...
        if (!str_cmp(arg, "voodoo") && (ch->level >= LEVEL_IMMORTAL))
        {
...
void do_vload(CHAR_DATA * ch, char *argument)
{
        ...
        for (d = descriptor_list; d != NULL; d = d->next)
        {
                ...
                if (!str_prefix(arg1, wch->name) && !found)
                {
                        ...
                        if ((pObjIndex =
                             get_obj_index(OBJ_VNUM_VOODOO_DOLL)) == NULL)
                        {
                                send_to_char
                                        ("Cannot find the voodoo doll vnum.\n\r",
                                         ch);
                                return;
                        }
                        obj = create_object(pObjIndex, 0);
                        name = wch->name;
                        xprintf(buf, "%s voodoo doll", name);
                        free_string(obj->name);
                        obj->name = str_dup(buf);

                        xprintf(buf, "a voodoo doll of %s", name);
                        free_string(obj->short_descr);
                        obj->short_descr = str_dup(buf);

                        xprintf(buf, "A voodoo doll of %s lies here.", name);
...
void do_oload(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char("Syntax: oload <vnum> <amount> <level>.\n\r",
                             ch);
                return;
        }
        ...
        else
        {
                ...
                {
                        send_to_char
                                ("Syntax: oload <vnum> <amount> <level>.\n\r",
                                 ch);
                        return;
                }
        ...
        else
        {
                ...
                if (i > 1)
                {
                        ...
                        act("$n pull a whole bunch of $p from their ass!", ch,
                            obj, NULL, TO_ROOM);
                        act("You pull a whole bunch of $p out of your ass!.",
                            ch, obj, NULL, TO_CHAR);
                        xprintf(buf, "$N loads %d of %s.", i,
                                obj->short_descr);
                        wiznet(buf, ch, NULL, WIZ_LOAD, WIZ_SECURE,
                               get_trust(ch));
                        return;
                }
        ...
        if (CAN_WEAR(obj, ITEM_TAKE))
        {
                ...
                {
                default:
                        act("$n pulls $p out of their ass!", ch, obj, NULL,
                            TO_ROOM);
                        break;
                case 1:
                        act("$n weaves their butthair into $p!", ch, obj,
                            NULL, TO_ROOM);
                        break;
                case 2:
                        act("$n pulls $p out of their nose!", ch, obj, NULL,
                            TO_ROOM);
                        break;
                case 3:
                        act("$n pulls $p out of their ear!", ch, obj, NULL,
                            TO_ROOM);
                        break;
                case 4:
                        act("$n takes a crap and finds $p in the turd!", ch,
                            obj, NULL, TO_ROOM);
                        break;
                }
        }
        else
        {
                ...
                {
                default:
                        act("$n drops $p out of their ass!", ch, obj, NULL,
                            TO_ROOM);
                        break;
                case 1:
                        act("$n weaves their butthair into $p!", ch, obj,
        ...
        default:
                ...
                break;
        case 1:
                act("You weave your butthair into $p!", ch, obj, NULL,
                    TO_CHAR);
                break;
        case 2:
        ...
        case 3:
                ...
                break;
        case 4:
                act("You take a crap and find $p in the #yturd#n!", ch, obj,
                    NULL, TO_CHAR);
                break;
        }
        ...
        xprintf(buf, "$N loads %s.", obj->short_descr);
        wiznet(buf, ch, NULL, WIZ_LOAD, WIZ_SECURE, get_trust(ch));
        return;
}
...
void do_trust(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Please enter: None, Builder, Questmaker, Enforcer, Judge, or Highjudge.\n\r",
                         ch);
                return;
        }
        ...
        if (get_trust(ch) >= MAX_LEVEL - 2 && !str_cmp(arg, "all"))
        {
                ...
                for (d = descriptor_list; d != NULL; d = d->next)
                {
...

                        {
                                ...
                                update_pos(victim);
                                if (victim->in_room != NULL)
                                        act("$n has restored you.", ch, NULL,
                                            victim, TO_VICT);

                        }
                }
                send_to_char("All active players restored.\n\r", ch);
                wiznet("All active players restored.\n\r", ch, NULL,
                       WIZ_RESTORE, WIZ_SECURE, get_trust(ch));
                return;
        }
...
void do_freeze(CHAR_DATA * ch, char *argument)
{
        ...
        else
        {
                ...
                send_to_char
                        ("You recieve a BITCHSLAP, and start crying like the bitch you are!\n\r",
                         victim);
...
//  send_to_char( "You can use channels again.\n\r", victim );
                ...
                xprintf(buf, "$N has removed silence for %s", victim->name);
                wiznet(buf, ch, NULL, WIZ_PENALTIES, WIZ_SECURE, 0);
        }
        else
        {
...
//  send_to_char( "You can't use channels!\n\r", victim );
                ...
                xprintf(buf, "$N has silenced %s", victim->name);
                wiznet(buf, ch, NULL, WIZ_PENALTIES, WIZ_SECURE, 0);
        }
...
void do_wizlock(CHAR_DATA * ch, char *argument)
{
        ...
        else
        {
                ...
                wiznet("$N has unwizlocked the game.", ch, NULL, 0, 0, 0);
        }
        return;
}
...
void do_mset(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
        {
                ...
                send_to_char("\n\r", ch);
                send_to_char("#wField being one of:\n\r", ch);
                send_to_char("#0  str int wis dex con sex level exp kingrank\n\r", ch);
                ...
                send_to_char
                        ("#0  name short long description title spec#n\n\r",
                         ch);
                return;
        }
        ...
        else if (!str_cmp(arg2, "toughness"))
        {
                ...
                {
                        send_to_char("Toughness range is 50 to 100.\n\r", ch);
                        return;
                }

                if (IS_JUDGE(ch))
                {
                        victim->pcdata->damreduct = value;
                        victim->pcdata->damreductdec = 0;
                        send_to_char("Ok.\n\r", ch);
                }
                else
                        send_to_char("Sorry, no can do...\n\r", ch);
	...
	if (!str_cmp(arg2, "race"))
	{
        ...
        if (!str_cmp(arg2, "hp"))
        {
                ...
                {
                        send_to_char
                                ("Hp range is 1 to 300,000 hit points.\n\r",
                                 ch);
                        return;
                }
        ...
        if (!str_cmp(arg2, "mana"))
        {
                ...
                {
                        send_to_char
                                ("Mana range is 0 to 300,000 mana points.\n\r",
                                 ch);
                        return;
                }
                if (IS_JUDGE(ch) || (ch == victim) || (IS_NPC(victim)))
        ...
        if (!str_cmp(arg2, "move"))
        {
                ...
                {
                        send_to_char
                                ("Move range is 0 to 300,000 move points.\n\r",
                                 ch);
                        return;
                }
                if (IS_JUDGE(ch) || (ch == victim) || (IS_NPC(victim)))
...
void do_oset(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
        {
                ...
                send_to_char("#wString being one of:\n\r", ch);
                send_to_char
                        ("#Y  name short long ed type extra wear owner\n\r",
                         ch);
                send_to_char("\n\r", ch);
        ...
        {
                send_to_char
                        ("You don't have permission to change that item.\n\r",
                         ch);
                return;
        }
        ...
        if (!str_cmp(arg2, "value0") || !str_cmp(arg2, "v0"))
        {
                ...
                {
                        send_to_char
                                ("You are not authorised to create spell weapons.\n\r",
                                 ch);
                        return;
                }
...
/*	if (obj->item_type == ITEM_WEAPON && value > 10)
        ...
        if (!str_cmp(arg2, "value3") || !str_cmp(arg2, "v3"))
        {
                if (obj->item_type == ITEM_ARMOR && !IS_JUDGE(ch))
                        send_to_char
                                ("You are not authorised to create spell armour.\n\r",
                                 ch);
                else
                {
                        obj->value[3] = value;
        ...
        if (!str_cmp(arg2, "morph"))
        {
                ...
                act("$p morphs into $P in $n's hands!", ch, obj, morph,
                    TO_ROOM);
                act("$p morphs into $P in your hands!", ch, obj, morph,
                    TO_CHAR);
                mnum = obj->wear_loc;
                obj_from_char(obj);
        ...
        if (!str_cmp(arg2, "extra"))
        {
                if (!str_cmp(arg3, "glow"))
                        ...
                        value = ITEM_LOYAL;
                else if (!str_cmp(arg3, "noclaim"))
                        value = ITEM_NOCLAIM;
                else if (!str_cmp(arg3, "nolocate"))
                        value = ITEM_NOLOCATE;
                else
                {
                        send_to_char
                                ("Extra flag can be from the following: Glow, Hum, Thrown, Vanish, Invis, Magic, Nodrop, Bless, Anti-Good, Anti-Evil, Anti-Neutral, Noremove, Inventory, Loyal, Noclaim, Nolocate.\n\r",
                                 ch);
                        return;
                }
                 ...
                 */
                if (IS_SET(obj->extra_flags, value) && value == ITEM_MAGIC
                    && !IS_JUDGE(ch))
                {
                        send_to_char("Sorry, no can do...\n\r", ch);
                        return;
        ...
        if (!str_cmp(arg2, "wear"))
        {
                ...
                else if (!str_cmp(arg3, "familiar"))
                        value = ITEM_WEAR_FAMILIAR;
                else
                {
                        send_to_char
                                ("Wear location can be from: None, Take, Finger, Neck, Body, Head, Legs, Hands, Arms, About, Waist, Hold, Face, Medal, Bodyart, Familiar.\n\r",
                                 ch);
                        return;
                }
                if (IS_SET(obj->wear_flags, ITEM_TAKE))
        ...
        if (!str_cmp(arg2, "level"))
        {
                ...
                if (!IS_JUDGE(ch))
                        send_to_char
                                ("You are not authorised to change an items level.\n\r",
                                 ch);
                else
                {
                        obj->level = value;
        ...
        if (!str_cmp(arg2, "type"))
        {
                ...
                {
                        send_to_char
                                ("You are not authorised to change an item type.\n\r",
                                 ch);
                        return;
                }
        ...
        if (!str_cmp(arg2, "owner"))
        {
                ...
                {
                        send_to_char
                                ("Someone else has already changed this item.\n\r",
                                 ch);
                        return;
                }
                ...
                {
                        send_to_char
                                ("You cannot find any player by that name.\n\r",
                                 ch);
                        return;
                }
                if (IS_NPC(victim))
...
void do_omni(CHAR_DATA * ch, char *argument)
{
        ...
        xprintf(buf,
                " Name         |Level|Trust|Gen|  Hit | Mana | Move |  HR|  DR|   AC| Bones\n\r");
        send_to_char(buf, ch);
        xprintf(buf,
                "--------------|-----|-----|---|------|------|------|----|----|-----|--------\n\r");
        send_to_char(buf, ch);
        ...
        for (d = descriptor_list; d != NULL; d = d->next)
        {
                ...
                xprintf(buf,
                        "%-14s|%5d|%5d|%3d|%6d|%6d|%6d|%4d|%4d|%5d|%8d\n\r",
                        wch->pcdata->switchname, wch->level, wch->trust,
                        wch->generation, wch->max_hit, wch->max_mana,
                        wch->max_move, char_hitroll(wch), char_damroll(wch),
                        char_ac(wch), wch->bones);
...
void do_users(CHAR_DATA * ch, char *argument)
{
        ...
        for (d = descriptor_list; d != NULL; d = d->next)
        {
                ...
                if (d->character != NULL && can_see(ch, d->character))
                {
                        ...
                        {
                                xcatf(buf,
                                      "#Y[#w%3d %16s#Y]  #R%s#G@#Y%s\n\r",
                                      d->descriptor, st,
                                      d->original ? d->original->pcdata->
                                      switchname : d->character ? d->
                                      character->pcdata->
                                      switchname : "(none)", d->host);
                        }
                        else
                        {
                                xcatf(buf,
...
void do_invis(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0')
                ...
                if (ch->invis_level)
                {
                        ...
                        {
                                act("$n appears in a blinding #f#Wflash#n!",
                 ...
                 */
        {
                ...
                {
                        send_to_char
                                ("Invis level must be between 1 and your level.\n\r",
                                 ch);
                        return;
                }
                else
...
void do_incognito(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0')
                ...
                {
                        ch->incog_level = 0;
                        act("$n is no longer cloaked.", ch, NULL, NULL,
                            TO_ROOM);
                ...
                else
                {
                        ...
                        ch->ghost_level = 0;
                        act("$n cloaks $s presence.", ch, NULL, NULL,
                            TO_ROOM);
                 ...
                 */
        {
                ...
                {
                        send_to_char
                                ("Incog level must be between 1 and your level.\n\r",
                                 ch);
                        return;
                }
                else
...
void do_ghost(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0')
                ...
                {
                        ch->ghost_level = 0;
                        act("$n steps out from the mist.", ch, NULL, NULL,
                            TO_ROOM);
                ...
                else
                {
                        ...
                        ch->incog_level = 0;
                        act("$n vanishes into a mist.", ch, NULL, NULL,
                            TO_ROOM);
                 ...
                 */
        {
                ...
                {
                        send_to_char
                                ("Ghost level must be between 1 and your level.\n\r",
                                 ch);
                        return;
                }
                else
...
void do_safe(CHAR_DATA * ch, char *argument)
{
        ...
        if (IS_SET(ch->in_room->room_flags, ROOM_SAFE))
                send_to_char
                        ("You cannot be attacked by other players here.\n\r",
                         ch);
        else
                send_to_char
                        ("You are not safe from player attacks in this room.\n\r",
                         ch);
        ...
        {
                send_to_char
                        ("You are inside, which means you are safe from sunlight.\n\r",
                         ch);
                return;
        }
        ...
        {
                send_to_char
                        ("It is not yet light out, so you are safe from the sunlight...for now.\n\r",
                         ch);
                return;
        }
        ...
        {
                send_to_char
                        ("This room is dark, and will protect you from the sunlight.\n\r",
                         ch);
                return;
        }

        send_to_char
                ("This room will provide you no protection from the sunlight.\n\r",
                 ch);
        return;
}
...
//                        SET_BIT(obj->spectype, SITEM_PULL);
                ...
                else if (!str_cmp(arg3, "delay1")
                         && IS_SET(obj->spectype, SITEM_DELAY1))
                        REMOVE_BIT(obj->spectype, SITEM_DELAY1);
                else if (!str_cmp(arg3, "delay1")
                         && !IS_SET(obj->spectype, SITEM_DELAY1))
                        SET_BIT(obj->spectype, SITEM_DELAY1);
                else if (!str_cmp(arg3, "delay2")
                         && IS_SET(obj->spectype, SITEM_DELAY2))
                        REMOVE_BIT(obj->spectype, SITEM_DELAY2);
                else if (!str_cmp(arg3, "delay2")
                         && !IS_SET(obj->spectype, SITEM_DELAY2))
                        SET_BIT(obj->spectype, SITEM_DELAY2);
        else if ( !str_cmp( arg3, "basic" ) && !IS_SET(obj->spectype, SITEM_COMMON) )
            SET_BIT(obj->spectype, SITEM_COMMON);
        else if ( !str_cmp( arg3, "unid" ) && !IS_SET(obj->spectype, SITEM_UNID) )
            SET_BIT(obj->spectype, SITEM_UNID);
        else if ( !str_cmp( arg3, "rare" ) && !IS_SET(obj->spectype, SITEM_RARE) )
            SET_BIT(obj->spectype, SITEM_RARE);
        else if ( !str_cmp( arg3, "wrare" ) && !IS_SET(obj->spectype, SITEM_RAREWEAP) )
            SET_BIT(obj->spectype, SITEM_RAREWEAP);
	else if ( !str_cmp( arg3, "rune" ) && !IS_SET(obj->spectype, SITEM_RUNE) )
...
void do_oclone(CHAR_DATA * ch, char *argument)
{
        ...
        if (!IS_JUDGE(ch)
            && (obj->questmaker == NULL || str_cmp(ch->name, obj->questmaker)
...
void do_evileye(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0' || arg2[0] == '\0')
        {
                ...
                send_to_char
                        ("Option TOGGLE has values: spell, self, other.\n\r\n\r",
                         ch);
                send_to_char
                        ("Using actions that will cause players to quit, lose connection\n\r",
                         ch);
                send_to_char
                        ("or spend experience on anything (train commands)\n\r",
                         ch);
                send_to_char
                        ("is not allowed, and you will regret using such an evileye.\n\r\n\r",
                         ch);
        ...
        else if (!str_cmp(arg1, "toggle"))
        {
                ...
                {
                        send_to_char
                                ("TOGGLE flag should be one of: spell, self, other.\n\r",
                                 ch);
                        return;
                }
                xprintf(buf, "%s flag toggled.\n\r", capitalize(arg2));
...
void do_artifact(CHAR_DATA * ch, char *argument)
{
        ...
        for (obj = object_list; obj != NULL; obj = obj->next)
        {
                ...
                if (in_obj->carried_by != NULL)
                {
                        xprintf(buf2, "%s", PERS(in_obj->carried_by, ch));
                }
                else
                {
                        xprintf(buf2, "in %d", in_obj->in_room->vnum);
                }
                ...
                {
                        xprintf(buf,
                                "%s, #0which has been discovered by #B%s#n.\n\r",
                                obj->short_descr, PERS(in_obj->carried_by,
                                                       ch));
                ...
                {
                        xprintf(buf,
                                "%s, which has not yet been discovered.\n\r",
                                obj->short_descr);
                }
...
void do_locate(CHAR_DATA * ch, char *argument)
{
        ...
        if (!found)
                send_to_char
                        ("You cannot locate any items belonging to you.\n\r",
                         ch);
...
void do_snatch(CHAR_DATA * ch, char *argument)
{
        ...
        if (victim->level >= ch->level
            && str_cmp(ch->pcdata->switchname, "Zarius"))
        {
                send_to_char("You Failed.\r\n", ch);
                return;
        }
...
void do_create(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Error...missing object, please inform KaVir.\n\r",
                         ch);
                return;
        }
...
void do_token(CHAR_DATA * ch, char *argument)
{
        ...
        else
        {
                ...
                {
                        send_to_char
                                ("Bone tokens should have a value between 1 and 100000.\n\r",
                                 ch);
                        return;
                }
                ...
                {
                        xprintf(buf,
                                "You only have %d bones left to put into tokens.\n\r",
                                ch->bones);
                        send_to_char(buf, ch);
                        return;
                }
        ...
        {
                send_to_char
                        ("Error...missing object, please inform Trace.\n\r",
                         ch);
                return;
        }
        ...
        {
                act("You reach behind $N's ear and produce $p.", ch, obj,
                    victim, TO_CHAR);
                act("$n reaches behind $N's ear and produces $p.", ch, obj,
                    victim, TO_NOTVICT);
                act("$n reaches behind your ear and produces $p.", ch, obj,
                    victim, TO_VICT);
        }
        else
        {
                act("You snap your fingers and reveal $p.", ch, obj, NULL,
                    TO_CHAR);
                act("$n snaps $s fingers and reveals $p.", ch, obj, NULL,
                    TO_ROOM);
        }
        return;
}
...
void do_clearstats(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You can only clear your stats at the Temple Altar of Midgaard.\n\r",
                         ch);
                return;
        }
    ...
    /* Dragon check */      //GryphonHawk
    if (IS_CLASS(ch, CLASS_DRAGON) && IS_POLYAFF(ch, POLY_DRAGON)) do_drackform (ch, "");    
...
void do_otransfer(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You don't have permission to otransfer that item.\n\r",
                         ch);
                return;
        }
        ...
        {
                act("$p vanishes from your hands in an explosion of energy.",
                    obj->carried_by, obj, NULL, TO_CHAR);
                act("$p vanishes from $n's hands in an explosion of energy.",
        ...
        else if (obj->in_room != NULL)
        {
                ...
                act("$p vanishes from the ground in an explosion of energy.",
                    ch, obj, NULL, TO_ROOM);
                if (chroom == objroom)
        ...
        act("$p appears in your hands in an explosion of energy.", victim,
            obj, NULL, TO_CHAR);
        act("$p appears in $n's hands in an explosion of energy.", victim,
            obj, NULL, TO_ROOM);
        return;
}
...
void do_bind(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You cannot do this while they are polymorphed.\n\r",
                         ch);
                return;
        }
        ...
        send_to_char("Ok.\n\r", ch);
        act("$n transforms into a white vapour and pours into $p.", victim,
            obj, NULL, TO_ROOM);
        act("You transform into a white vapour and pour into $p.", victim,
            obj, NULL, TO_CHAR);
...
void do_release(CHAR_DATA * ch, char *argument)
{
        ...
        act("A white vapour pours out of $p and forms into $n.", victim, obj,
            NULL, TO_ROOM);
        act("Your spirit floats out of $p and reforms its body.", victim, obj,
            NULL, TO_CHAR);
        return;
}
...
void do_hreload(CHAR_DATA * ch, char *argument)
{
        ...
        for (i = 0; i < 9; i++)
        {
                ...
                {
                        if ((found =
                             read_entry(ch, fp, helpfile[i], arg)) == TRUE)
                        {
                                fclose(fp);
                                break;
                        }
...
void do_resetpassword(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You can't change the password of that player.\n\r",
                         ch);
                return;
        }
...
void copyover_update(void)
{
        ...
        for (gch = char_list; gch != NULL; gch = gch->next)
        {
                ...
                {
                        REMOVE_BIT(gch->pcdata->powers[GOLEMS_SUMMON],
                                   HAS_SUMMONED_FIRE);
                        REMOVE_BIT(gch->pcdata->powers[GOLEMS_SUMMON],
                                   HAS_SUMMONED_IRON);
                        REMOVE_BIT(gch->pcdata->powers[GOLEMS_SUMMON],
        ...
        for (d = descriptor_list; d; d = d->next)
        {
                ...
                if (gch->desc->out_compress)
                {
                        if (!compressEnd2(gch->desc))
                                send_to_char
                                        ("Could not disable compression, you'll have to reconect in 5 seconds.\n",
                                         gch);
                }
        }
...
void do_implag(CHAR_DATA * ch, char *argument)
{
        ...
        else
        {
                ...
                else if (IS_AFF2(victim, PLR_IMPLAG))
                {
                        ...
                        send_to_char(buf, ch);
                        xprintf(buf,
                                "You suddenly look at the world normally.\n\r");
                        send_to_char(buf, victim);
                        return;
                }
                else
...
void do_toggle(CHAR_DATA * ch, char *argument)
{
        ...
        if (!strcmp(argument, "qp") && !strcmp(argument, "chaos")
            && !strcmp(argument, "cp") && !strcmp(argument, "dt")
            && !strcmp(argument, "exp") && !strcmp(argument, "extra_log"))
        {
                send_to_char("Syntax: toggle <bones|exp|cp|dt|all> <on|off>.\n\r",
                             ch);
                return;
        }
        if (!str_cmp(arg, "exp"))
        {
                if (!str_cmp(arg2, "on"))
                {
                        ...
                        {
                                send_to_char
                                        ("Double exp is already in affect!\n\r",
                                         ch);
                                return;
                        }
                        global_exp = TRUE;
        ...
        if (!str_cmp(arg, "bones"))
        {
                if (!str_cmp(arg2, "on"))
                {
                        ...
                        {
                                send_to_char
                                        ("Double bones is already in affect!\n\r",
                                         ch);
                                return;
                        }
                        global_qp = TRUE;
                        xprintf(buf,
                                "%s has declared double bones for everyone \n\r",
                                ch->name);
                        do_info(ch, buf);
        ...
        if (!str_cmp(arg, "dt"))
        {
                if (!str_cmp(arg2, "on"))
                {
                        ...
                        {
                                send_to_char
                                        ("Super Training is now in affect!\n\r",
                                         ch);
                                return;
                        }
                        global_dt = TRUE;
                        xprintf(buf,
                                "#R#fSuper Training#n #wNOW in affect!\n\r");
                        do_info(ch, buf);
        ...
        if (!str_cmp(arg, "cp"))
        {
                if (!str_cmp(arg2, "on"))
                {
                        ...
                        {
                                send_to_char
                                        ("Double Class Points is already in affect!\n\r",
                                         ch);
                                return;
                        }
                        global_cp = TRUE;
                        xprintf(buf,
                                "%s has declared double Class Points for everyone \n\r",
                                ch->name);
                        do_info(ch, buf);
                        return;
                }
                if (!str_cmp(arg2, "off"))
                {
                        ...
                        {
                                send_to_char
                                        ("The global cp flag isn't on!.\n\r",
                                         ch);
                                return;
                        }
                        global_cp = FALSE;
                        xprintf(buf,
                                "%s has removed double class points!\n\r",
                                ch->name);
                        do_info(ch, buf);
                        return;
                }
        ...
        if (!str_cmp(arg, "chaos"))
        {
                if (!str_cmp(arg2, "on"))
                {
                        ...
                        {
                                send_to_char
                                        ("Chaos is already in affect!\n\r",
                                         ch);
                                return;
                        }
                        global_chaos = TRUE;
                        xprintf(buf,
                                "%s throws the world into #sC#sH#sA#sO#sS#n\n\r",
                                ch->name);
                        do_info(ch, buf);
                        return;
                }
                if (!str_cmp(arg2, "off"))
                {
                        ...
                        {
                                send_to_char
                                        ("The global chaos flag isn't on!.\n\r",
                                         ch);
                                return;
                        }
                        global_chaos = FALSE;
                        xprintf(buf,
                                "%s restores #YOrder#n to the world#n\n\r",
                                ch->name);
                        do_info(ch, buf);
                        return;
                }
        }
        send_to_char("Syntax: toggle <exp|bones|cp|dt|chaos|all> <on|off>\n\r", ch);
}
