...
//        signal(SIGTERM, panic_shutdown);
        ...
        xprintf(buf, "Cotn 4 is ready to rock on port %d.", port);
...
//                            signal(SIGSEGV, SIG_IGN); raise(SIGSEGV); return;}
        		    ...
        		    xprintf(exp_str, "%2.0f", ch->exp);
                            ...
                            xprintf(buf,
                                    "#R-#0=%s#0=#R- #R-#0=Hp#R:#Y%s #0Mana#R:#Y%s #0Move#R:#Y%s#0=#R- #n",
		            ...
		            xprintf(exp_str, "%2.0f", ch->exp);
                            xprintf(buf,
                                    "#R-#0=#0Exp#R:#Y%s#0=#R- #R-#0=Hp#R:#Y%s #0Mana#R:#Y%s #0Move#R:#Y%s#0=#R- #n",
...
xprintf(buf2, "%d", ch->max_hit); ADD_COLOUR(ch, buf2, YELLOW); i = buf2; break; case 'm':
xprintf(buf2, "%d", ch->mana); COL_SCALE(buf2, ch, ch->mana, ch->max_mana); i = buf2; break; case 'M':
xprintf(buf2, "%d", ch->max_mana); ADD_COLOUR(ch, buf2, YELLOW); i = buf2; break; case 'v':
xprintf(buf2, "%d", ch->move); COL_SCALE(buf2, ch, ch->move, ch->max_move); i = buf2; break; case 'V':
xprintf(buf2, "%d", ch->max_move); ADD_COLOUR(ch, buf2, YELLOW); i = buf2; break; case 'x':
xprintf(buf2, "%2.0f", ch->exp); //COL_SCALE(buf2, ch, ch->exp, 1000); i = buf2; break; case 'X':
xprintf(buf2, "\n\r"); i = buf2; break; case 'q':
xprintf(buf2, "%d", ch->bones); ADD_COLOUR(ch, buf2, YELLOW); i = buf2; break; case 'Q':
                            if (!IS_NPC(ch))
                            xprintf(buf2, "%d",
