...
char     *format_obj_to_char(OBJ_DATA * obj, CHAR_DATA * ch, bool fShort)
{
	...
	if ( IS_SET(obj->quest2, QUEST2_PALADINBLESS)) 
        ...
        if (obj->points < 750 && obj->points != 0)
                strcat(buf, "#Y(Legendary)#n ");
        if (obj->points < 1250 && obj->points != 0)
...
int char_damroll(CHAR_DATA * ch)
{
	...
	value += (ch->pcRaceLevel * 5);
        ...
        if (IS_SET(ch->special, SPC_WOLFMAN) && ch->rage > 99)
                value += ch->rage;
        if (IS_CLASS(ch, CLASS_WEREWOLF)) value += 500;
  ...
  if (IS_CLASS(ch, CLASS_ANTIPALADIN) && IS_SET(ch->pcdata->powers[15], APOWER_MIGHT)) value += 600;
        if (value > 10000)
                value = 10000;
        return value;
}
...
void show_char_to_char_0(CHAR_DATA * victim, CHAR_DATA * ch)
{
    ...
    if ( !IS_NPC(victim) && !IS_NPC(ch) && IS_CLASS(victim, CLASS_ANTIPALADIN) && IS_HERO(victim) && IS_HERO(ch))
        strcat( buf, "#R(#0Anti-Paladin#R)#n " );
...
void do_score(CHAR_DATA * ch, char *argument)
{
        ...
        {
                sprintf(buf,
                        "#R[#nYou have #C%d#n out of #C%d #npoints of Anti-Paladin power stored#R]\n\r",
                        ch->pcdata->stats[DEMON_CURRENT],
                        ch->pcdata->stats[DEMON_TOTAL]);
                send_to_char(buf, ch);
        }
    ...
    {
        sprintf( buf,
        "#R[#nYou have #C%d#n out of #C%d #npoints of unholy power stored#R]\n\r",
				...
				break;
                        case MAX_LEVEL - 1:
                                if (IS_EXTRA(wch, EXTRA_AFK))
                                        ...
                                        class = wch->prefix;
                                         ...
                                         (wch->pcdata->switchname, "Xrakisis"))
                        ...
                        case MAX_LEVEL - 9:
                                if (wch->prefix != NULL)
                                        ...
                                        class = "   #R*#0***A#RF#0K***#R*#n  ";
                                else if (IS_SET
                                         (wch->pcdata->jflags, JFLAG_BULLY))
                                        class = "  #0B#Rully#n       ";
          else if (mightRate > 4000 ) class = "   #7Almighty#n     ";
          else if (mightRate > 3750 ) class = "   #CRuler#n        ";
          else if (mightRate > 3500 ) class = "   #YSupreme#n      ";
          else if (mightRate > 3000 ) class = "   #pKing#n         ";
          else if (mightRate > 2750 ) class = "   #RBaron#n        ";
          else if (mightRate > 2250 ) class = "   #yDuke#n         ";
          else if (mightRate > 2000 ) class = "   #gGeneral#n      ";
          else if (mightRate > 1750 ) class = "   #CCaptain#n      ";
          else if (mightRate > 1500 ) class = "   #RMaster#n       ";
          else if (mightRate > 1250 ) class = "   #yLegendary#n    ";
          else if (mightRate > 1000 ) class = "   #GHero#n         ";
          else if (mightRate >  750 ) class = "   #LAdventurer#n   ";
          else if (mightRate >  500 ) class = "   #rVeteran#n      ";
          else if (mightRate >= 150 ) class = "   #oCadet#n        ";
          else class = "   #pNewbie#n       ";
     ...
     */
		...
		if (mightRate > 3500)
		...
		else
			{
                ...
                if (!mains)
                {
//                        sprintf(buf, " %s\n\r", generate__banner("", 76));
  sprintf(buf, " %s\n\r", generate_banner( "#YCOTN 4.0", 80, "#0" ) );
                        send_to_char(buf, ch);

//send_to_char("#c                      o---O        o            o   o#n\n\r",ch);
//send_to_char("#l                      |            |            |   |#n\n\r",ch);
//send_to_char("#L                      O     o--o o-O-o o-o      o__ o#n\n\r",ch);
//send_to_char("#l                      |     |  |   |   |  |         |#n\n\r",ch);
//send_to_char("#c                      o---O o--o   o   o  o         o#n\n\r",ch);
send_to_char("#R			+---X        x          x  x\n\r",ch);
send_to_char("#0			|            |          |  |\n\r",ch);
send_to_char("#R			x     +--+ x-+-x x---x  x--x\n\r",ch);
send_to_char("#0			|     |  |   |   |   | 	   |\n\r",ch);
send_to_char("#R			+---X +--+   x   X   x     x#n\n\r",ch);
                        mains = TRUE;
                }
...
if ( dSock->connected != CON_PLAYING || !dSock || !dSock->character )
     ...
     if ( pCh->level < 6 )
       ...
       nImmVis++;
        ...
        sprintf(buf,
                "       #Y%d#R/#Y%d #0Visible players and #Y%d #0visible immortals connected to %s#n\n\r",
...
//  send_to_char(buf, ch);
  ...
  b = a / 24;
  c = a - b * 24;
  sprintf(buf, " #R[#0Statistics Reset#R:#Y %d #0days and #Y%d #0hours ago#R][#0Average Online#R:#Y %d#R][#0Peak Online#R:#Y %d#R]#n\n\r",
...
void do_finger( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ( fp = fopen( strsave, "r" ) ) != NULL )
  {
	...
	sprintf(buf2,"#WAnti-Paladin ");
        break;
      case CLASS_ZOMBIE:
	sprintf(buf2,"#WZombie       ");
	break;
      case CLASS_BLADE_MASTER:
	sprintf(buf2,"#WBladeMaster  ");
	break;
    }
    send_to_char(buf2,ch);
