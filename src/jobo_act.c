...
//        xprintf(buf, " %s\n\r", get_dysopia_banner("  Bountylist  ", 60));
  sprintf(buf, " %s\n\r", generate_banner( " Bountylist ", 80, "#0" ) );
...
void do_top(CHAR_DATA * ch, char *argument)
{
        ...
        if (IS_NPC(ch))
                return;
  sprintf(buf, " %s\n\r", generate_banner( " Top Pkscore ", 80, "#0" ) );
        send_to_char(buf, ch);
...
void do_mastery(CHAR_DATA * ch, char *argument)
{
		...
		vnum = 407;
	else if (IS_CLASS(ch, CLASS_ANTIPALADIN))
		vnum = 408;
        else
        {
                send_to_char
...
void load_leaderboard()
{
  ...
  {   
    log_string(LOG_GAME,"Error: leader.txt not found!");
    exit(1);
  }
...
void save_leaderboard()
{
  ...
  {
    log_string(LOG_GAME,"Error writing to leader.txt");
    return;
  }
...
{    
  ...
  sprintf(buf, " %s\n\r", generate_banner( " Leader Board", 80, "#0" ) );
        send_to_char(buf, ch);
...
//                send_to_char(buf, ch);
   ...
   if (!IS_NPC(ch) && (IS_CLASS( ch, CLASS_ANTIPALADIN)))
    {
        sprintf( buf, "  #R[#rYou have #0%d#r out of #0%d #rpoints of unholy power stored#R]#n\n\r",
