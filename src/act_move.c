...
void do_train(CHAR_DATA * ch, char *argument)
{
   ...
   if( !str_cmp( arg1, "rank" ) ) {
      ...
      ch->exp -= rank_xp;
      ch->pRank++;
      sprintf( buf, "%s is now known as %s %s", ch->pcdata->switchname, getCurrRanking( ch, ch->pRank ), 
ch->pcdata->switchname );
      do_info( ch, buf );
      return;
   }
...
else if (!str_cmp(arg1, "toughness") && IS_SET(ch->newbits, NEW_MASTERY))
{
  if (IS_NPC(ch))
    return;
    ...
    if (IS_CLASS(ch, CLASS_ELEMENTAL))
       {
        ...
        if (!IS_NPC(ch) && IS_SET(ch->newbits, NEW_MASTERY))
        {
          if (ch->pcdata->damreduct > 50 || (ch->pcdata->damreduct == 50 && ch->pcdata->damreductdec > 0))
          {
            sprintf(buf, "#wToughness #R- #w2 billion exp to lower your damage taken to #B%d#D.#B%d%%.#n\n\r", ch->pcdata->damreductdec == 0 ? ch->pcdata->damreduct - 1 : ch->pcdata->damreduct, ch->pcdata->damreductdec == 0 ? 9 : ch->pcdata->damreductdec - 1);
            send_to_char(buf,ch);
          }
        }
