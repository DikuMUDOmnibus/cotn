...
void do_noset(CHAR_DATA * ch, char *argument)
{
        ...
        send_to_char("Ok.\n\r", ch);
        send_to_char
                ("All your custom messages have been removed, and you can no longer change your messages.\n\r",
                 victim);
        return;
}
...
void do_specfind(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("This command can crash the mud if used with the wrong argument, don't use it!\n\r",
                         ch);
                return;
        }
...
void do_itemwearfind(CHAR_DATA * ch, char *argument)
{
        ...
        for (i = 1; i < 200000; i++)
        {
                ...
                if (idx != NULL)
                {
                        if (IS_SET(idx->wear_flags, value))
                        {
                                xprintf(buf,
                                        "Object %s (%d) has the flag set\n\r",
                                        idx->name, i);
                                send_to_char(buf, ch);
                        }
                }
        ...
        for (d = descriptor_list; d != NULL; d = d->next)
        {
                ...
                for (d2 = d->next; d2 != NULL; d2 = d2->next)
                {
                        ...
                        if (!str_cmp(d->host, d2->host))
                        {
                                ...
                                found = TRUE;
                                xprintf(buf,
                                        "%s and %s are multiplaying bitches\n\r",
                                        d2->character->name,
                                        d->character->name);
                                send_to_char(buf, ch);
                        }
...
void do_showsounds(CHAR_DATA * ch, char *argument)
{
        ...
        if (!found)
                send_to_char("Noone uses sounds.\n\r", ch);
        return;
}
...
void do_faith(CHAR_DATA * ch, char *argument)
{
        ...
        for (d = descriptor_list; d; d = d->next)
        {
                ...
                {
                        if (gch->pcdata->faith < 1)
                                continue;
                        xprintf(buf, "%-14s - %3d/%-3d faithpoints\n\r",
                                gch->name, gch->pcdata->current_faith,
                                gch->pcdata->faith);
                        send_to_char(buf, ch);
                }
        }
...
void do_newallow(CHAR_DATA * ch, char *argument)
{
        ...
        prev = NULL;
        for (curr = newbieban_list; curr != NULL;
...
void do_ccenter(CHAR_DATA * ch, char *argument)
{
        ...
        {
                xprintf(buf,
                        "  #0[#G***#0]   #YMindcloud Control Center   #0[#G***#0]#n\n\r\n\r");
                send_to_char(buf, ch);
                xprintf(buf,
                        "  #R(#0lexp#R) #YMinimum Experience        #G%7d#n\n\r",
                        ccenter[CCENTER_MIN_EXP]);
                send_to_char(buf, ch);
                xprintf(buf,
                        "  #R(#0uexp#R) #YMaximum Experience        #G%7d#n\n\r",
                        ccenter[CCENTER_MAX_EXP]);
                send_to_char(buf, ch);
                xprintf(buf,
                        "  #R(#0elvl#R) #YExperience Level              #G%3d#n\n\r",
                        ccenter[CCENTER_EXP_LEVEL]);
                send_to_char(buf, ch);
                xprintf(buf,
                        "  #R(#0qlvl#R) #YQuestpoint Level              #G%3d#n\n\r",
                        ccenter[CCENTER_QPS_LEVEL]);
                send_to_char(buf, ch);
                send_to_char
...
void do_aggrocheck(CHAR_DATA * ch, char *argument)
{
        ...
        send_to_char
                ("#0Name          From    Towards   Counter  Bully   BC#n\n\r",
                 ch);
        for (gch = char_list; gch; gch = gch->next)
        {
                ...
                xprintf(buf, "%-12s  %4d       %4d       %3d    %3d  %3d\n\r",
                        gch->name, gch->pcdata->aggress_from,
                        gch->pcdata->aggress_towards,
...
void do_psilence(CHAR_DATA * ch, char *argument)
{
        ...
        send_to_char("They are now silenced, this action was logged.\n\r",
                     ch);
        xprintf(buf, "%s was silenced by our local police %s.", victim->name,
                ch->name);
...
void do_plog(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Please state how many actions should be logged (between 10 and 100).\n\r",
                         ch);
                return;
        }
