...
*/
     void rare_eq1 ( CHAR_DATA * ch, CHAR_DATA * victim )
        ...
        hitroll = number_range( 180, 200   );
        damroll = number_range( 180, 200   );
	...
	SET_BIT( obj->spectype, SITEM_UNID );
     ...
     void rare_weapon1 ( CHAR_DATA * ch, CHAR_DATA * victim )
        ...
        add_affect( obj, APPLY_MOVE, move );
        add_affect( obj, APPLY_AC, armor );
        obj->value[1] = number_range (75,100);
        obj->value[2] = number_range (100,125);
	...
	return;
        ...
        if (obj->questowner != NULL && strlen(obj->questowner) > 1 && str_cmp(ch->pcdata->switchname, obj->questowner))
        {
        stc("YOU CAN NOT SELL WHAT YOU DO NOW OWN... FOOL!\n\r", ch);
        return;
        }
...
*/
	if (obj->pIndexData->vnum >= 11059 && obj->pIndexData->vnum <= 11066)        
	{
        sprice = number_range( 10000, 16000 );
        obj_from_char( obj );
        ch->bones += sprice;
	...
	if ( IS_NPC( ch ) ) return;
 	...
 	if (is_safe(ch,victim)) return;
  	...
  	do_info(ch, buf);
  	log_string(LOG_GAME, "%s shown mercy by %s at %d.",victim->pcdata->switchname,ch->pcdata->switchname);
